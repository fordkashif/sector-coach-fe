...
Deno.serve(async (request) => {
  ...
  if (!invite) return json(404, { error: "Coach invite not found." })
  if (invite.status !== "pending") return json(400, { error: "Coach invite is not pending." })
  if ((invite.email ?? "").trim().toLowerCase() !== email) {
    return json(400, { error: "This invite is for a different email address." })
  }
  if (invite.expires_at && new Date(invite.expires_at).getTime() < Date.now()) {
    return json(400, { error: "Coach invite has expired." })
  }
